...
class LLMController:
    ...
    def __init__(self, 
                 backend: LanguageModelProvider = LanguageModelProvider.LANGUAGE_MODEL_PROVIDER_LITELLM_SERVER,
                 model: str = "gpt-4", 
                 api_key: Optional[str] = None):
        if backend == LanguageModelProvider.LANGUAGE_MODEL_PROVIDER_OPENAI:
            self.llm = OpenAIController(model, api_key)
        elif backend == LanguageModelProvider.LANGUAGE_MODEL_PROVIDER_OLLAMA:
            self.llm = OllamaController(model)
        elif backend == LanguageModelProvider.LANGUAGE_MODEL_PROVIDER_LITELLM_SERVER:
            self.llm = LiteLLMController()
        else:
            raise ValueError("Backend must be either 'openai', 'ollama', or 'litellm'. ")
...
class MemoryNote:
    ...
    def analyze_content(content: str, llm_controller: LLMController) -> Dict:            
        ...
        class AnalyzeContentSignature(dspy.Signature):
            ...
            keywords: List[str] = dspy.OutputField(
                desc = "Several specific, distinct keywords that capture key concepts and terminology. Order from most to least important. Don't include keywords that are the name of the speaker or time. At least three keywords, but don't be too redundant."
            )
            context: str = dspy.OutputField(
                desc = "One sentence summarizing: Main topic/domain, Key arguments/points, and Intended audience/purpose."
            )
            tags: List[str] = dspy.OutputField(
                desc = "Several broad categories/themes for classification. Include domain, format, and type tags. At least three tags, but don't be too redundant."
            )
        try:
...
class AgenticMemorySystem:
    ...
    def __init__(self, 
                 ...
                 llm_backend: LanguageModelProvider = LanguageModelProvider.LANGUAGE_MODEL_PROVIDER_LITELLM_SERVER,
    ...
    def process_memory(self, note: MemoryNote) -> bool:
        ...
        # Eventually need to convert OpenAI and OLLama to using dspy.
        class ProcessMemorySignature(dspy.Signature):
            """
            You are an AI memory evolution agent responsible for managing and evolving a knowledge base.
            Analyze the the new memory note according to keywords and context, also with their several nearest neighbors memory.
            Make decisions about its evolution.  
            ...
            should_evolve: bool = dspy.OutputField(
                desc = "Should this memory be evolved? Consider its relationships with other memories."
            )
            actions: List[Literal['strengthen', 'update_neighboor']] = dspy.OutputField(
                desc = "What specific actions should be taken (strengthen, update_neighbor)"
            )
            suggested_connections: List[str] = dspy.OutputField(
                desc = "If choose to strengthen the connection, which memory should it be connected to?"
            )
            tags_to_update: List[str] = dspy.OutputField(
                desc = "Can you give the updated tags of this memory?"
            )
            # Might need to make optional
            new_context_neighborhood: List[str] = dspy.OutputField(
                desc = "If choose to update_neighbor, you can update the context of these memories based on the understanding of these memories. If the context is not updated, the new context should be the same as the original ones. Generate the new context in the sequential order of the input neighbors."
            )
            # Might need to make optional
            new_tags_neighborhood: List[List[str]] = dspy.OutputField(
                desc = "If choose to update_neighbor, you can update the tags of these memories based on the understanding of these memories. If the tags are not updated, the new tags should be the same as the original ones. Generate the new tags in the sequential order of the input neighbors. Tags should be determined by the content of these characteristic of these memories, which can be used to retrieve them later and categorize them. Note that the length of new_tags_neighborhood must equal the number of input neighbors, and the length of new_context_neighborhood must equal the number of input neighbors."
            )
        response = self.llm_controller.llm.get_completion(prompt_memory, response_format=ProcessMemorySignature)
        # try:
...
def run_tests():
    ...
    memory_system = AgenticMemorySystem(
        model_name='all-MiniLM-L6-v2',
        llm_backend=LanguageModelProvider.LANGUAGE_MODEL_PROVIDER_LITELLM_SERVER,
    ...
    def log_to_obj(s: str):
        # Allow any whitespace (tabs/spaces/newlines) between pairs
        pattern = r'([^:\n\t]+):\s*(.*?)(?=\s{2,}[^:\n\t]+:|$)'
        data = {}
        ...
        for k, v in re.findall(pattern, s):
            ...
            if key.startswith("memory "):
                key = key[len("memory "):].strip()

            # Safely interpret lists/numbers when possible
            try:
    ...
    for i, memory in enumerate(related, 1):
        memory_updated = log_to_obj(memory)
        print(memory_updated)
        print(f"\n{i}. Memory:")
        print(f"Content: {memory_updated.content}")
        print(f"Category: {memory_updated.category}")
        print(f"Keywords: {memory_updated.keywords}")
        print(f"Tags: {memory_updated.tags}")
        print(f"Context: {memory_updated.context}")
        print("-" * 50)
